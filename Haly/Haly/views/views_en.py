...
def home(req):
    ...
    return render(req, 'en/index-1.html', {'gallery_home': gallery_home})
...
def news_one(req, title):
    filtered_news = NewsModel.objects.filter(slug_title_en=title).all()
    recent_news = NewsModel.objects.order_by('-pk').exclude(slug_title_en=title).all()[:3]
    ...
    return render(req, 'en/news/news_one.html', context)
...
def blog_post(req, title):
    all_blogs = BlogModel.objects.order_by('-pk').exclude(slug_title_en=title).all()[:3]
    blog = BlogModel.objects.filter(slug_title_en=title).all()
    ...
    return render(req, 'en/blog/blog.html', context)
